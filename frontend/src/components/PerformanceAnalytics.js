...
const PerformanceAnalytics = () => {
  ...
  useEffect(() => {
    // Load static and summary data once on component mount
    loadPlayers();
    loadAttendanceSummary();
  }, []);

  useEffect(() => {
    // Load player-specific data when the selectedPlayer state changes
    if (selectedPlayer) {
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-3xl font-bold mt-1 ${getAttendanceColor(attendanceSummary.overall_attendance_rate)}`}>
                  {attendanceSummary.overall_attendance_rate ? attendanceSummary.overall_attendance_rate.toFixed(1) : 'N/A'}%
                </p>
              </div>

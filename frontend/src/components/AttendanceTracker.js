...
const AttendanceTracker = () => {
  ...
  const updatePerformanceRating = (playerId, value) => {
    setAttendanceRecords(prevRecords =>
      prevRecords.map(record =>
        record.player_id === playerId
          ? {
              ...
              performance_ratings: {
                ...
                overall: value ? parseInt(value) : null,
              },
            }
          : record
      )
  ...
  const markAllAbsent = () => {
    setAttendanceRecords(prevRecords =>
      prevRecords.map(record => ({ ...record, present: false, performance_ratings: {} }))
    );
  };
  ...
  const getAverageRating = () => {
      const ratings = attendanceRecords
        .filter(r => r.present && r.performance_ratings?.overall)
        .map(r => r.performance_ratings.overall);
  ...
  const getRatingDistribution = () => {
      const ratings = attendanceRecords
          ...
          .map(record => record.performance_ratings?.overall)
          .filter(r => r != null);
      const distribution = {};
      for (let i = 1; i <= 10; i++) {
    ...
    <div className={`p-6 space-y-6 ${isLight ? 'bg-gray-50' : 'bg-black'} min-h-screen`}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className={`${isLight ? 'text-gray-600' : 'text-gray-400'} mt-1`}>Track and manage player attendance and performance for training sessions</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className={`${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-cyan-500/30'} rounded-2xl p-6 shadow-sm transition-all duration-200 hover:shadow-md`}>
        ...
        <div className={`${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-green-500/30'} rounded-2xl p-6 shadow-sm transition-all duration-200 hover:shadow-md`}>
          <div className="flex items-center justify-between">
            <div>
        ...
        <div className={`${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-red-500/30'} rounded-2xl p-6 shadow-sm transition-all duration-200 hover:shadow-md`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${isLight ? 'text-gray-600' : 'text-gray-400'}`}>Absent</p>
              <p className={`text-3xl font-bold ${isLight ? 'text-red-600' : 'text-red-400'} mt-1`}>{getAbsentCount()}</p>
              <p className={`text-sm ${isLight ? 'text-gray-500' : 'text-gray-400'} mt-1`}>Missing today</p>
            </div>
            <div className={`p-3 rounded-xl ${isLight ? 'bg-red-100' : 'bg-red-500/20'}`}><XCircle className={`w-6 h-6 ${isLight ? 'text-red-600' : 'text-red-400'}`} /></div>
          </div>
        </div>
        <div className={`${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-purple-500/30'} rounded-2xl p-6 shadow-sm transition-all duration-200 hover:shadow-md`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${isLight ? 'text-gray-600' : 'text-gray-400'}`}>Avg Performance</p>
              <p className={`text-3xl font-bold ${isLight ? 'text-purple-600' : 'text-purple-400'} mt-1`}>{getAverageRating()}/10</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className={`${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-cyan-500/30'} rounded-2xl p-6 shadow-sm`}>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie data={getAttendanceData()} cx="50%" cy="50%" outerRadius={80} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
        ...
        <div className={`${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-purple-500/30'} rounded-2xl p-6 shadow-sm`}>
          <h3 className={`text-lg font-semibold ${isLight ? 'text-gray-900' : 'text-white'} mb-4`}>Performance Ratings</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={getRatingDistribution()}>
              ...
              <Bar dataKey="count" fill={isLight ? '#8B5CF6' : '#a855f7'} radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex flex-wrap gap-3">
          <button onClick={markAllPresent} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-200 ${isLight ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-green-500/20 text-green-400 border border-green-500/30 hover:bg-green-500/30'}`}><CheckCircle className="w-4 h-4" />Mark All Present</button>
          <button onClick={markAllAbsent} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-200 ${isLight ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30'}`}><XCircle className="w-4 h-4" />Mark All Absent</button>
        </div>
      ...
      {message && <div className={`p-4 rounded-xl ${isLight ? 'bg-green-50 border border-green-200 text-green-800' : 'bg-green-500/10 border border-green-500/30 text-green-400'}`}>{message}</div>}
      {error && <div className={`p-4 rounded-xl ${isLight ? 'bg-red-50 border border-red-200 text-red-800' : 'bg-red-500/10 border border-red-500/30 text-red-400'}`}>{error}</div>}

      <div className={`${isLight ? 'bg-white border border-gray-200' : 'bg-gray-900 border border-cyan-500/30'} rounded-2xl shadow-sm overflow-hidden`}>
          ...
          <table className="w-full">
            <thead className={`${isLight ? 'bg-gray-50' : 'bg-gray-800/50'}`}>
              <tr className={`border-b ${isLight ? 'border-gray-200' : 'border-cyan-500/20'}`}>
                ...
                <th className={`text-center font-medium py-4 px-6 ${isLight ? 'text-gray-900' : 'text-white'}`}>Performance (1-10)</th>
            ...
            <tbody>
              {filteredRecords.map((record) => (
                <tr key={record.player_id} className={`border-b transition-colors duration-200 ${isLight ? 'border-gray-100 hover:bg-gray-50' : 'border-cyan-500/10 hover:bg-cyan-500/5'}`}>
                  ...
                  <td className="py-4 px-6 text-center">
                    <select value={record.performance_ratings?.overall || ''} onChange={(e) => updatePerformanceRating(record.player_id, e.target.value)} disabled={!record.present}
                      className={`w-24 px-3 py-2 rounded-lg border text-center transition-all duration-200 ${isLight ? 'border-gray-200 bg-white text-gray-900 focus:border-blue-500' : 'border-purple-500/30 bg-gray-800 text-white focus:border-purple-400'} focus:outline-none focus:ring-2 focus:ring-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed`}
                    >
                  ...
                  <td className="py-4 px-6">
                    <input type="text" value={record.notes} onChange={(e) => updateAttendanceRecord(record.player_id, 'notes', e.target.value)} placeholder="Add notes..."
                      className={`w-full px-3 py-2 rounded-lg border transition-all duration-200 ${isLight ? 'border-gray-200 bg-gray-50 text-gray-900 focus:bg-white focus:border-blue-500' : 'border-gray-600/30 bg-gray-800 text-white focus:border-gray-400'} focus:outline-none focus:ring-2 focus:ring-blue-500/20`}
                    />
                  </td>
                </tr>
              ))}

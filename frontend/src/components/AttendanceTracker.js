...
import { 
  Calendar, Users, UserCheck, TrendingUp, Clock, 
  CheckCircle, XCircle, Search, Filter, Save
} from 'lucide-react';
...
const AttendanceTracker = () => {
  ...
  const getRatingDistribution = () => {
    const ratings = attendanceRecords
      .filter(record => record.present && record.performance_rating)
      .map(record => record.performance_rating);

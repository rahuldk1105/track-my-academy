...
const Avatar = ({ name }) => {
  ...
  return (
    <div className="w-10 h-10 rounded-none bg-gradient-to-br from-emerald-500 to-teal-500 text-white flex items-center justify-center font-semibold">
      {initials || 'CH'}
    </div>
  );
};
  ...
  return (
    <div className={`${isLight ? 'bg-white border border-gray-200 shadow-sm hover:shadow-md' : 'bg-gray-900 border border-white/10 hover:bg-gray-800'} rounded-none transition-all duration-200 p-4 flex flex-col`}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
        ...
        <div>
          <span className={`px-2 py-0.5 rounded-none text-xs font-medium border ${coach.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-gray-100 text-gray-700 border-gray-200'}`}>

        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center">
              ...
              <div className={`flex items-center ${isLight ? 'bg-gray-100' : 'bg-white/10'} rounded-xl px-4 py-2 mr-6`}>
                <img 
                  src={academyLogo || "https://i.ibb.co/1Z8cJ6q/academy-default-logo.png"} 
                  alt={`${academyData?.name || 'Academy'} Logo`} 
                  className={`h-12 w-12 rounded-lg object-cover mr-3 ${isLight ? 'border-gray-300' : 'border-white/20'} border shadow-lg`}
                  onError={(e) => {
                    e.target.src = "https://i.ibb.co/1Z8cJ6q/academy-default-logo.png";
                  }}
                />
                <div>
                  <div className={`${isLight ? 'text-gray-800' : 'text-white'} font-semibold text-lg`}>
                    {academyData?.name || 'Academy'}
                  </div>
                  <div className={`${isLight ? 'text-gray-600' : 'text-sky-400'} text-sm font-medium`}>
                    Academy Portal
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'players' && (
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className={`text-xl font-semibold ${isLight ? 'text-gray-800' : 'text-white'}`}>Player Management</h2>
                <button
                  onClick={() => {
                    setEditingPlayer(null);
                    setShowPlayerModal(true);
                  }}
                  className={`${isLight ? 'bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100' : 'bg-blue-500/20 text-blue-400 border-blue-500/30 hover:bg-blue-500/30'} border px-4 py-2 rounded-lg transition-all duration-300`}
                >
                  + Add Player
                </button>
              </div>

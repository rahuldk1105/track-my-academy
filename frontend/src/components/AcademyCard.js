...
const AcademyCard = ({ academy, selected, onSelect, onApprove, onReject, onEdit, onDelete }) => {
  ...
  return (
    <div className={`${isLight ? 'bg-white border border-gray-200 shadow-sm hover:shadow-md' : 'bg-gray-900 border border-white/10 hover:bg-gray-800'} rounded-none transition-all duration-200 p-4 relative`}>
      <div className="absolute top-3 left-3">
        <input type="checkbox" checked={selected} onChange={onSelect} className={`${isLight ? 'rounded-none border-gray-300 text-sky-600 focus:ring-sky-500' : 'rounded-none bg-gray-800 border-gray-600 text-sky-500 focus:ring-sky-500'}`} />
      </div>
      <div className="flex items-start gap-3 pl-7">
        {logoUrl ? (
          <img src={logoUrl} alt={`${academy.name} logo`} className={`${isLight ? 'bg-gray-100' : 'bg-gray-800'} w-12 h-12 rounded-none object-cover border ${isLight ? 'border-gray-200' : 'border-white/10'}`} />
        ) : (
          <div className={`${isLight ? 'bg-gray-100' : 'bg-gray-800'} w-12 h-12 rounded-none flex items-center justify-center`}>
        ...
        <span className={`px-2 py-0.5 rounded-none text-xs font-medium border ${
